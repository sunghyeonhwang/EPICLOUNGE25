...
if($_SESSION["final_idx"] == ""){
    echo "<script>alert('입력정보가 유효하지 않거나 만료되었습니다. 다시 시도하여주세요. 결제가 취소되었습니다.');location.href=\"/v3/unrealfest2024_test/application_step1.html\";</script>";
    exit;
}
...
if ($_REQUEST["P_STATUS"] === "00") {             // 인증이 P_STATUS===00 일 경우만 승인 요청
    ...
    if($code == "STD_ALL" || $code == "NORMAL_ALL"){
        ...
        $sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_track = '".$track."' and apply_temp_yn = 'N'  and (apply_product_code like '%28' or apply_product_code like '%ALL')";
        $result_cnt = sql_fetch($sql);

    }else if($code == "STD_29" || $code == "NORMAL_29"){
        $sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_track = '".$track."' and apply_temp_yn = 'N'  and (apply_product_code like '%29' or apply_product_code like '%ALL')";
        $result_cnt = sql_fetch($sql);
    }
...
//문자 발송 종료
            ...
            //echo "location.href='/v3/unrealfest2024_test/application_form.html';";
            echo "history.back(-1);";
            echo '</script>';
            exit();
        }
