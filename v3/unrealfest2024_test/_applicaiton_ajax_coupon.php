...
if($apply_product_code == "STD_ALL" || $apply_product_code == "STD_28" || $apply_product_code == "STD_29"){
    ...
    $sql = "select * from cb_unreal_2024_event2_coupon where coupon_serial = '".$apply_user_coupon."' and coupon_type = '학생'";
    $result = sql_fetch($sql);
}else if($apply_product_code == "NORMAL_ALL" || $apply_product_code == "NORMAL_28" || $apply_product_code == "NORMAL_29"){

    $sql = "select * from cb_unreal_2024_event2_coupon where coupon_serial = '".$apply_user_coupon."' and coupon_type = '일반'";
    $result = sql_fetch($sql);
}else{
    echo "잘못된 접근입니다.";
    exit;
