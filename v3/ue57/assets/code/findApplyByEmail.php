...
if ($mode == 'write') {
  ...
  $sql = "select count(*) from cb_unreal_2021_summit_apply where apply_user_email = '$apply_user_email' limit 1";
  ...
  $sql = "select count(*) from cb_unreal_2021_summit_apply where apply_user_phone = '$apply_user_phone' limit 1";
  $result = $mysqli -> query($sql);
  $obj = $result -> fetch_array();
  if($obj[0]['count']){
    echo '<script type="text/javascript">';
  ...
  $sql = "INSERT INTO cb_unreal_2021_summit_apply (
      apply_user_name, 
      apply_user_email,
      apply_user_phone,
      apply_user_job, 
...
$ch = curl_init();
$title = "언리얼 엔진 5.1 출시 웨비나";
$message = '[$NAME]님, <언리얼 엔진 5.1 출시 웨비나>에 등록되었습니다.
2022년 11월 29일, 오후 1시부터 등록된 메일과 카카오톡 문자로 발송되는 URL로 접속하거나 에픽 라운지에 방문하셔서 시청하실 수 있습니다.
 - 에픽 라운지 사무국';             //필수입력
...
//문자 발송 종료
  ...
  echo '<script type="text/javascript">';
  echo "alert('등록이 완료되었습니다. \\n시청 URL 주소는 11월 29일 오후 1시, \\n카카오톡 및 메일로 발송될 예정입니다.'); location.replace('/');";
  echo '</script>';
  exit();
  ...
  $sql = "select count(*) from cb_unreal_2021_summit_apply where apply_user_email = '$apply_user_email' and apply_no != '$apply_no' limit 1";
  $result = $mysqli -> query($sql);
  $obj = $result -> fetch_array();
  if($obj[0]['count']){
    echo '<script type="text/javascript">';
  ...
  $sql = "select count(*) from cb_unreal_2021_summit_apply where apply_user_phone = '$apply_user_phone' and apply_no != '$apply_no' limit 1";
  $result = $mysqli -> query($sql);
  $obj = $result -> fetch_array();
  if($obj[0]['count']){
    echo '<script type="text/javascript">';
  ...
  $sql = "UPDATE cb_unreal_2021_summit_apply SET
  ...
  $sql = "select apply_user_email,apply_user_phone,apply_password from cb_unreal_2021_summit_apply where apply_no = '$apply_no' limit 1";
  ...
  $sql2 = "UPDATE cb_unreal_2021_summit_apply SET
  ...
  $mysqli -> close();
  echo '<script type="text/javascript">';
  echo "location.replace('https://epiclounge.co.kr');";
  echo '</script>';
  exit();
  ...
  $mysqli = new mysqli("localhost","unrealengine","Good121930!@","unrealengine");
  $sql = "INSERT INTO cb_unreal_2021_summit_apply_inquire(
    apply_user_name, 
    apply_user_email,
    apply_user_phone,
    apply_user_company, 
