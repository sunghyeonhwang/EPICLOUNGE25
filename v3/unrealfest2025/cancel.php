...
if ($mode2 == 'del2') {
  ...
  $sql = "select * from cb_unreal_2025_event2_apply where apply_no = '$apply_no' limit 1";

  $result = $mysqli -> query($sql);
  $obj = $result -> fetch_array();

    if ($obj['apply_user_email'] != $email) {
        echo '<script type="text/javascript">';
        echo 'alert("등록정보가 일치하지 않습니다.");';
        echo 'history.back();';
        echo '</script>';
        exit();
    }

    if ($obj['apply_user_phone'] != $phone) {
        echo '<script type="text/javascript">';
        echo 'alert("등록정보가 일치하지 않습니다.");';
        echo 'history.back();';
        echo '</script>';
  ...
  $sql2 = "UPDATE cb_unreal_2025_event2_apply SET
  ...
  $result = $mysqli -> query($sql2);
    ...
    $sql3 = "delete from cb_unreal_2025_event2_apply_inquire where apply_user_email = '".$obj['apply_user_email']."'";
  ...
  echo "location.replace('https://epiclounge.co.kr/v3/unrealfest2025/index.html');";
  echo '</script>';
  exit();
  ...
  WHERE apply_no = '$apply_no'";
        ...
        sql_query("UPDATE cb_unreal_2025_event2_coupon SET 
                                        usage_date = null, 
                                        user_name = '', 
                                        user_email = '{$obj["apply_user_email"]}', 
                                        user_phone = '{$obj["apply_user_phone"]}' , 
                                        creator_memo = concat(creator_memo,', 사용자 등록 취소(,now(),)') 
                                    WHERE coupon_key = '{$obj["apply_coupon_no"]}'");
