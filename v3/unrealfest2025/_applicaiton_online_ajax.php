...

{
    ...
    if($dup['cnt'] > 0){
        header('Content-Type: application/json');
        echo json_encode(array("result" => false, "msg" => "1인 1회만 등록 가능합니다. 이미 등록하신 온라인 등록 또는 오프라인 등록을 취소하신 후 다시 등록하세요."));
        exit;
    }
    ...
    $title = "<언리얼 페스트 서울 2025> 등록";
    $message = "<언리얼 페스트 서울 2025>\n온라인 등록이 완료되었습니다.\n\n일시: 8월 25일(월) 10시 30분/8월 26일(화) 9시 30분 ~\n\n채널: 에픽 라운지\n\n행사 당일 오전 카카오톡 / 이메일로 접속 링크가 발송됩니다.\n시청 링크를 별도로 받지 못한 경우, 행사 당일 본 페이지에 접속하여 등록 시 사용한 이메일 주소로 체크인하시면 시청하실 수 있습니다.\n등록정보 변경 및 취소는 FAQ를 참조해 주세요.\n\n-언리얼 페스트 사무국";
    ...
    header('Content-Type: application/json');
    echo json_encode(array("result" => true, "msg" => "성공"));
    exit;
