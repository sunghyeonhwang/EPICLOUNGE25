...
?>
  <div class="memeber_container">
      <div class="wrap">
       ...
       <div style="text-align: right; padding: 10px 0 ; font-weight: bold; font-size: 1.2em;"> <span style="color:red">*</span> 트랙 선택에 따라 입장이 우선됩니다.</div>
				  </div>
...
<!-- 트랙이 가득차있을경우 body에 modal_on 클래스  -->
  ...
  <script>
        $(document).ready(function() {
            ...
            // Handle disabled track radio buttons
            $(document).on('click', '.track-disabled', function (e) {
                e.preventDefault();
                alert('선택하신 트랙의 판매가 모두 마감되었습니다.');
            });
...
<script>
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.result == true) {
                    ...
                    $("input.track1[value='" + track + "']").addClass('track-disabled');

                    $("input.track1[value='" + track + "']").next('label').addClass('track-disabled-label');
                    $("input.track1[value='" + track + "']").next('label').on('click', function (e) {
                        e.preventDefault();
                        alert('선택하신 트랙의 판매가 모두 마감되었습니다.');
                    });
    ...
    function fn_pay_select_normal(type) {
        ...
        if (type == 1) {
            $("#apply_product_name").val("일반 양일권");
            $("#apply_product_price").val("120000");
            $("#apply_product_code").val("NORMAL_ALL");
