...
$result = sql_query("select * from cb_unreal_2024_event2_apply where free_yn = 'N' and (apply_pay_status = 10 or apply_pay_status = 1) and send_sms_yn='N' limit 10");

$row_cnt = sql_fetch("select count(*) cnt from cb_unreal_2024_event2_apply where free_yn = 'N' and (apply_pay_status = 10 or apply_pay_status = 1) and send_sms_yn='N'");
/*echo "<table>";
...
for ($i=1; $row=sql_fetch_array($result); $i++) {
    if($i <= 10){
        ...
        $file = array();
        $file[] = array('attc' => 'https://epiclounge.co.kr/v3/unrealfest2024/qrdata/'.$row['apply_no'].'.jpg');
        ...
        if(curl_errno($ch)){
            ...
            sql_query("update cb_unreal_2024_event2_apply set send_sms_yn = 'Y' where apply_no = '".$row['apply_no']."'");
        }
        curl_close ($ch);
    }else{
