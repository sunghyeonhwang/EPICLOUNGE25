...
$dup2 = sql_fetch("select count(*) as cnt from cb_unreal_2024_event2_apply where  apply_user_name = '$apply_user_name' and  apply_user_email = '$apply_user_email' and apply_user_phone = '$apply_user_phone'  and (apply_temp_yn = 'Y' OR apply_temp_yn is null) ");
if($dup2['cnt'] > 0){
    if(empty($_apply_ci) and empty($_apply_di) and empty($last_idx)){
        $dup3 = sql_fetch("select * from cb_unreal_2024_event2_apply where  apply_user_name = '$apply_user_name' and  apply_user_email = '$apply_user_email' and apply_user_phone = '$apply_user_phone'  and (apply_temp_yn = 'Y' OR apply_temp_yn is null) ");
...
{
    $dup = sql_fetch("select count(*) as cnt from cb_unreal_2024_event2_apply where apply_user_name = '$apply_user_name' and  apply_user_email = '$apply_user_email' and apply_user_phone = '$apply_user_phone' ");
    if($dup['cnt'] > 0){
        echo "중복된 신청입니다.";
        exit;
    }
