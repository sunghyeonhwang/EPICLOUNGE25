...
sql_query("insert into 2023_event_log(log_text,rdate) values('".str_replace("'","`",$requestAsString)."',now())");

if($_REQUEST["P_STATUS"]=="02"){
    $rdata = sql_fetch("select * from cb_unreal_2023_event_apply where mpay_oid = '".$_REQUEST["P_OID"]."' limit 1");
    if($_REQUEST["P_AMT"] >= $rdata["mpay_amt"]){

        $sql = "UPDATE cb_unreal_2023_event_apply SET pay_complete = 'Y',apply_pay_status=10  WHERE mpay_oid = '".$_REQUEST["P_OID"]."'";
        sql_query($sql);
        ...
        $imagePath =  $documentRoot.'/v3/unrealfest2023/qrdata/'.$rdata["apply_no"].'.png';
        $imagePath2 =  $documentRoot.'/v3/unrealfest2023/qrdata/'.$rdata["apply_no"].'.jpg';
...
//sha256($_SESSION["final_idx"]);
        ...
        $file[] = array('attc' => 'https://epiclounge.co.kr/v3/unrealfest2023/qrdata/'. $rdata["apply_no"].".jpg");
