...
$rdata = sql_fetch("select * from cb_unreal_2023_event_apply where pay_moid = '".$_REQUEST["no_oid"]."' limit 1");
if($_REQUEST["amt_input"] >= $rdata["pay_totprice"]){

    $sql = "UPDATE cb_unreal_2023_event_apply SET pay_complete = 'Y',apply_pay_status=10  WHERE pay_moid = '".$_REQUEST["no_oid"]."'";
    sql_query($sql);
