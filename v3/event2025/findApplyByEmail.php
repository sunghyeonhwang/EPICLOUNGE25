...
if ($mode == 'findApplyByEmail') {
  ...
  $sql = "select count(*) from cb_unreal_2025_luman_apply where apply_user_email = '$apply_user_email' limit 1";
  ...
  if($obj[0]['count']){
    ...
    echo "alert('이미 등록된 메일입니다.'); location.replace('/event2025/complete-part-02.html'); ";
    echo '</script>';
    exit();
  }else{
    echo '<script type="text/javascript">';
    echo 'document.location.href="/event2025/notice-part.html"';
    echo '</script>';
    exit();
  };
  ...
  $sql = "INSERT INTO cb_unreal_2025_luman_apply (
      apply_title, 
      apply_sector,
      apply_file,
      apply_sns, 
  ...
  echo '<script type="text/javascript">';
  echo "location.replace('/event2025/complete-part.html');";
  echo '</script>';
  exit();
  ...
  $sql = "UPDATE cb_unreal_2025_luman_apply SET
...
WHERE apply_user_email = '$apply_user_email'";
  ...
  echo '<script type="text/javascript">';
  echo "location.replace('/event2025/complete-part-03.html');";
  echo '</script>';
  exit();
  ...
  $sql = "select apply_user_email from cb_unreal_2025_luman_apply where apply_no = '$apply_no' limit 1";

  $result = $mysqli -> query($sql);
  $obj = $result -> fetch_array();
  $mail =  'del:'.$obj['apply_user_email'];

  $sql2 = "UPDATE cb_unreal_2025_luman_apply SET
