...
### [2026-01-08-2] 슬라이드 및 뉴스 섹션 스타일링 조정

- **파일**: `v3/resource/css/main_layout26.css`
- **내용**:
  - **슬라이드 섹션 배경색 변경**: `.visual_slide` (상단 비주얼) 및 개별 슬라이드 배경색을 **흰색(`#fff`)**으로 설정.
  - **뉴스 섹션 하단 여백 방식 변경**: `.news_sec`의 하단 여백을 `margin-bottom`에서 **`padding-bottom: 100px`**로 변경 (PC/태블릿 기준).
  - **모바일 대응 여백 조정**: 480px 이하 모바일 기기에서는 `padding-bottom`을 **50px**로 축소하여 과도한 여백 방지.
  - **[Fix]** 모바일 미디어 쿼리(640px) 내 `.news_sec` 태그의 누락된 세미콜론 수정.
  - **반응형 대응**: 저해상도 기기(480px 이하)에서도 충분한 하단 여백이 유지되도록 스타일 조정.
  - **슬라이드 폰트 스타일 조정**:
    - 제목(`.bg_slide_title`): 폰트 크기를 본문과 동일한 **29px**로 맞추고 두께를 **800(Extra Bold)**으로 강화.
    - 본문(`.bg_slide_text`): 폰트 크기를 **29px**로 1px 축소하고 두께를 **400(Regular)**으로 변경.
    - 반응형: 1000px 이하(22px), 640px 이하(16px) 등 각 해상도별로 제목과 본문 크기를 동일하게 유지하도록 조정.
  - **슬라이드 폰트 종류 변경**:
    - 슬라이드 1 & 2의 제목, 본문, 버튼 폰트를 **Pretendard**로 변경 (font.css에 웹폰트 임포트 추가).
    - 사용자가 직접 조정한 폰트 사이즈(제목 42px/40px, 본문 24px)와 폰트 두께 스타일 유지.
