...
- **내용**:
  - `.bg_slide_box` 높이 **540px**로 변경 및 모바일 대응 (반응형 높이 조정).
  - 기존 DB 연동 슬라이드를 **하드코딩된 비디오 슬라이드 2개**로 대체.
  - 배경 영상(`.bg_video`)에 **밝기 필터(`brightness(0.3)`)**를 직접 적용하여 가독성 개선 (기존 오버레이 방식 제거).
  - 슬라이드 자동 재생(6초) 설정 추가.
  - **[Fix]** 슬라이드 컨테이너의 상단 패딩(`70px`)을 제거(`0px`)하여 콘텐츠를 정중앙에 배치.
  - **[Fix]** 타이틀 및 일반 텍스트 폰트 사이즈를 각각 2px씩 축소 (48px / 30px).
