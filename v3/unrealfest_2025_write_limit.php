...
<meta property="og:type" content="website" />
    ...
    <meta property="og:title" content="언리얼 페스트 서울 2025 서울 발표자 모집" />
    <meta property="og:image" content="https://unrealsummit16.cafe24.com/2025/unrealfest2025/ufest25_key_image.jpg" />
    <meta property="og:image:height" content="630px" />
    <meta property="og:image:width" content="1200px" />
    <meta property="og:description" content="오는 8월25일과 26일 양일간, 코엑스에서 진행되는 <언리얼 페스트 서울 2025>의 발표자를 모집합니다!">
...
<script>
  ...
  const earlyRedirectDate = new Date('2025-03-20T00:00:00');
  const lateRedirectDate = new Date('2025-09-01T00:00:00');

  if (now < earlyRedirectDate) {
    alert("언리얼 페스트 서울 2025 스피커 모집 페이지 입니다. 4월 21일 이후 등록이 가능하며, 링크의 공유는 불가합니다.\n Registration will be available after April 21, and the link must not be shared.");
    window.location.href = "https://epiclounge.co.kr/";
  } else if (now >= lateRedirectDate) {
    alert("언리얼 페스트 2025 스피커, 등록이 마감되었습니다.");
    window.location.href = "https://epiclounge.co.kr/";
  }
</script>
<!-- container -->
...
if(isset($_REQUEST['speaker_ph']) && $_REQUEST['speaker_ph'] != ""){
    $RData = sql_fetch("select * from cb_unreal_2025_speaker_apply where speaker_ph = '{$_REQUEST['speaker_ph']}' and speaker_email = '{$_REQUEST['speaker_email']}'");
    if($RData['id'] == ""){
...
<div id="event_write">
    ...
    <div class="wrap">
        ...
        <div class="write_box">
            <form action="unrealfest_2025_write_proc.php" id="frmWrite" name="frmWrite" method="post" enctype="multipart/form-data">
                <fieldset>
                    ...
                    <div class="in_text">
                        ...
                        <div class="input_file">
                            <p class="file_text">Max 2MB, 이미지 파일/2인 이상의 경우 압축해서 zip파일로 업로드주세요.</p>
                    ...
                    <script>
                        document.getElementById('speaker_pic').addEventListener('change', function(event) {
                            let fileName = event.target.files[0] ? event.target.files[0].name : 'Max 2MB, 이미지 파일/2인 이상의 경우 압축해서 zip파일로 업로드주세요.';
  ...
  !@#$%^*()_-=+ ‘” 등의 특수문자는 사용이 어렵습니다. / !@#$%^*()_-=+ ‘” Special characters cannot be used."><?php echo isset($RData['speaker_hi']) ? htmlspecialchars($RData['speaker_hi']) : ''; ?></textarea>
                    ...
                    <div class="ch_list_box line">
                        ...
                        <ul class="list_4">
                            ...
                            <!-- <li>
                                <input type="checkbox" id="product_4" class="product" name="product_4" value="Epic Games Store"
                                    <?php if(isset($RData['product']) && in_array("Epic Games Store", explode(',', $RData['product']))) echo 'checked'; ?> />
                                <label tabindex="0" for="product_4">Epic Games Store</label>
                            </li> -->
                    ...
                    <div class="ch_list_box line">
                        ...
                        <ul class="list_3">
                            ...
                            <!-- <li>
                                <input type="checkbox" id="platform_2" class="platform" name="platform_2" value="맥 / Mac"
                                    <?php if(isset($RData['platform']) && in_array("맥 / Mac", explode(',', $RData['platform']))) echo 'checked'; ?> />
                                <label tabindex="0" for="platform_2">맥 / Mac</label>
                            </li> -->
                            <li>
...
ex) 1. Key Considerations for VR Content Creation
      ...
      4. Optimization Techniques for High-Quality Graphics and Immersive Gameplay in VR"><?php echo isset($RData['speaker_table']) ? htmlspecialchars($RData['speaker_table']) : ''; ?></textarea>
                        <!-- <p class="must mt_0">*필수입력 / Required field</p> -->
                    </div>
...
ex)Attendees will learn key considerations and precautions in VR content development. This will enable them to create more immersive and high-quality VR content while understanding optimization techniques for performance, a crucial issue in VR."><?php echo isset($RData['speaker_key']) ? htmlspecialchars($RData['speaker_key']) : ''; ?></textarea>
                    ...
                    <div class="ch_list_box line demo_next_pan " <?php if($RData['demo'] == "" || $RData['demo'] == "아니오/no"){?> style="display:none;" <?}?> >
                        ...
                        <ul class="list_2">
                            <!-- <li>
                                <input type="radio" id="does_1" name="does_1" value="시연 없음/No demo" <?php if(isset($RData['does_demo']) && $RData['does_demo'] == "시연 없음/No demo") echo 'checked'; ?> />
                                <label tabindex="0" for="does_1">시연 없음/No demo</label>
                            </li> -->
...
ex) The presentation includes videos, so clear audio playback is required."><?php echo isset($RData['speaker_requests']) ? htmlspecialchars($RData['speaker_requests']) : ''; ?></textarea>
                    ...
                    <div class="btn_box_fot btn_list_3">
                            ...
                            <button type="button"  onclick="location.href = '/unrealfest25_speaker.php';" class="btn_type1">취소 / Cancel</button>
...
<script type="text/javascript">
    ...
    $('.btn_type2').on('click', function(e) {
        ...
        //var doesChecked       = $('input[name="does_1"]:checked');
        ...
        // 데모가 "예/yes"일 경우, 인 에디터 시연 옵션이 반드시 선택되어야 함
...
<script>
    ...
    // compositionstart: IME 입력 시작 시 필터링 중지
    textarea.addEventListener('compositionstart', function(e) {
        isComposing = true;
    });

    // compositionend: IME 입력 완료 후 필터링 실행
    textarea.addEventListener('compositionend', function(e) {
        isComposing = false;
        filterInput();
    });

    // 일반 input 이벤트: composition 중이 아니라면 필터링
    ...
    function filterInput() {
        ...
        if (disallowedPattern.test(currentValue)) {
            alert("한글, 영어, 숫자, 공백과 일부 특수문자(.,&)만 입력 가능합니다.\nOnly Korean, English, numbers, whitespace, and . , & are allowed.");
            textarea.value = currentValue.replace(disallowedPattern, '');
        }
    }
    // function filterInput() {
    //     // 허용되는 문자: 영문(대소문자), 숫자, 한글(가-힣), 공백
    //     var disallowedPattern = /[^A-Za-z0-9가-힣ㄱ-ㅎ\s]/g;
    //     var currentValue = textarea.value;
    //     if (disallowedPattern.test(currentValue)) {
...
<form action="unrealfest_2025_delete_proc.php" id="frmDelete" method="post">
