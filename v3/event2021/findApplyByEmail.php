...
if ($mode == 'findApplyByEmail') {
  ...
  if($obj[0]['count']){
    ...
    echo "alert('이미 등록된 메일입니다.'); location.replace('/event2021/complete-part-02.html'); ";
    echo '</script>';
    exit();
  }else{
    echo '<script type="text/javascript">';
    echo 'document.location.href="/event2021/notice-part.html"';
    echo '</script>';
    exit();
  };
  ...
  echo '<script type="text/javascript">';
  echo "location.replace('/event2021/complete-part.html');";
  echo '</script>';
  exit();
  ...
  if($_FILES['file']['name']) {
    ...
    $savefile = '/unrealengine/www/event2021/file/' . $random_name;
...
WHERE apply_user_email = '$apply_user_email'";
  ...
  echo '<script type="text/javascript">';
  echo "location.replace('/event2021/complete-part-03.html');";
  echo '</script>';
  exit();
