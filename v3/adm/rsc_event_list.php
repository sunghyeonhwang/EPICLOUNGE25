...
if(!empty($_GET[sc_text])){
	...
	$sql_list_cnt = 
		" select count(*) cnt from v3_rsc_event_bbs 
		  where 1=1 " 
		  . $where . " ";
...
<div class="local_ov01 local_ov">
  <span class="btn_ov01"><span class="ov_txt">생성된 이벤트수</span><span class="ov_num"> <?=$cnt["cnt"]?>개</span></span>
</div>
...
<form name="frm_delete" id="frm_delete" action="./rsc_event_list_proc.php" method="post">
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" width="140px">카테고리</th>
        <th scope="col" width="140px">상태</th>
        <th scope="col"  width="180px">일정</th>
        <th scope="col" >제목</th>
...
?>
    ...
    <tr class="bg0" id="tr_<?=$row_list["rsc_bbs_idx"]?>">
        ...
        <td><?=$row_list["category"]?> </td>
        <td> <?=$row_list["status"]?></td>
        <td><?=$row_list["sdate"]?> ~ <?=$row_list["edate"]?></td>
        <td class="text_left"><?=$row_list["title"]?></td>
...
<?
	...
	if($row_list["site_url"]){
		...
		$link = "/v3/contents/event_view.php?rsc_bbs_idx=".$row_list["rsc_bbs_idx"];
	}
?>
        ...
        <td class="">
            <a href="./rsc_event_modify.php?rsc_bbs_idx=<?=$row_list["rsc_bbs_idx"]?>" class="btn btn_03 margin_b_5">수정</a><br />
...
<div class="btn_fixed_top">
        <input type="button" onclick="group_delete()" name="" value="선택삭제" onclick="" class="btn_02 btn">
            <a href="./rsc_event_write.php" id="bo_add" class="btn_01 btn">이벤트 추가</a>
    </div>
...
function group_delete(){
	if(confirm("정말로 선택한 이벤트를 삭제 하시겠습니까?")){
		$("#frm_delete").submit();
	}
}
$(function(){
...
<script>
		...
		$.ajax({
			...
			success : function(json) {
				if(mode == "delete"){
					...
					if(field == "category"){
						$.toast('<h4>성공!</h4> 해당 게시물은 변경된 카테고리로 이동하였습니다.. ', {
						  sticky: true,
						  duration: 2000,
						  type: 'success'
						});
    ...
    function move_to(mode, idx,field){
        $.ajax({
            ...
            data : {'mode':'event','idx': idx,'move':mode},
            dataType : 'json',
            success : function(json) {
