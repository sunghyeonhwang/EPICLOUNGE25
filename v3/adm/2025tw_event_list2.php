...
$sql_common = " from cb_unreal_2025_event_apply a ";
...
$sql_wait_cnt = " select count(*) as cnt from cb_unreal_2025_event_apply_inquire ";
...
?>
    ...
    <a href="2023_event_list.php?sc_md=free" class="btn btn_02 ft_11">단체포함 온라인   <?=$row_free_cnt["cnt"]?>명</a>
        <a href="2023_event_wait_list.php" class="btn btn_02 ft_11">대기   <?=$row_wait_cnt["cnt"]?>명</a>
        <a href="2023_event_list.php?sc_md=group" class="btn btn_02 ft_11">단체   <?=$row_group_cnt["cnt"]?>명</a>
        <a href="2023_event_list.php?sc_md=fr" class="btn btn_02 ft_11">외국인   <?=$row_fr_cnt["cnt"]?>명</a>
</div>
    ...
    <form name="fboardlist" id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
      ...
      <td><?=$row['apply_product_name']?></td> -->
                        ...
                        <td>
                            <a style="font-weight: 900;" href="javascript:;" onclick="<?=$row['free_yn'] == "Y"?"del":"del_pay"?>(<?=$row['apply_no']?>)">
                                삭제
                            </a>
                        </td>
                    </tr>
    ...
    <script type="text/javascript">
        ...
        function del(val){
            if (confirm('삭제 하시겠습니까?')) {
                location.replace('2023_findApplyByEmail.php?mode2=del&no='+val);
            }
        }
        function del_pay(val){
            if (confirm('삭제 및 환불처리를 진행 하시겠습니까?')) {
                location.replace('2023_findApplyByEmail.php?mode2=del2&no='+val);
            }
        }
    </script>
