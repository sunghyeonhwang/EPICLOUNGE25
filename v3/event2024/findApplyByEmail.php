...
if ($mode == 'findApplyByEmail') {
  ...
  if($obj[0]['count']){
    ...
    echo "alert('이미 등록된 메일입니다.'); location.replace('/event2024/complete-part-02.html'); ";
    echo '</script>';
    exit();
  }else{
    echo '<script type="text/javascript">';
    echo 'document.location.href="/event2024/notice-part.html"';
    echo '</script>';
    exit();
  };
  ...
  echo '<script type="text/javascript">';
  echo "location.replace('/event2024/complete-part.html');";
  echo '</script>';
  exit();
...
WHERE apply_user_email = '$apply_user_email'";
  ...
  echo '<script type="text/javascript">';
  echo "location.replace('/event2024/complete-part-03.html');";
  echo '</script>';
  exit();
